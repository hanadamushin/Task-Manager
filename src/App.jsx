...
function LLMModal({p, onClose}) {
  ...
  async function send(){const msg=input.trim();if(!msg||loading)return;setErr("");setInput("");const nextChat=[...chat,{role:"user",content:msg}];setChat(nextChat);setLoading(true);
    try{const data=await callClaude([{role:"user",content:sysPrompt()},...nextChat],{max_tokens:1000});const text=textOf(data);setChat(c=>[...c,{role:"assistant",content:text}]);try{const rows=normalize(parseJsonArray(text));if(rows.length)setPreview(rows);}catch(e2){console.log("JSON not found, continuing chat");}}catch(e){setErr(e.message||"APIエラー");}
    setLoading(false);}
